...
export function ErpPrimaryButton({ children, icon: Icon = Plus, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement> & { icon?: LucideIcon }) {
  return (
    <button
      {...props}
      className={cn(
        "flex items-center gap-2 px-5 py-3 bg-brand-primary hover:bg-brand-primary/95 text-white rounded-2xl text-xs font-black shadow-lg shadow-brand-primary/10 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed",
        props.className
      )}
    >
  ...
  return (
    <div className={cn(
      ...
      <div className="space-y-1">
        ...
        <h3 className={cn("text-2xl font-black", darkMode ? "text-white" : "text-slate-850")}>{value}</h3>
        {change && <span className="text-[10px] font-black text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded-md">{change}</span>}
      </div>
      <div className={cn(
...
export const ErpFilterTab: React.FC<ErpFilterTabProps> = ({ active, onClick, children }) => {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        "px-4 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all whitespace-nowrap",
        active
...
export function ErpModal({ title, onClose, children, maxWidth = "max-w-md" }: {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={cn(
        "relative w-full p-8 rounded-[2rem] border shadow-2xl space-y-6 transition-all duration-300 max-h-[90vh] overflow-y-auto",
        maxWidth,
  ...
  return (
    <thead>
      <tr className={cn(
        ...
        {columns.map((col) => (
          <th key={col} className="py-4.5 px-6 text-left">{col}</th>
        ))}
      </tr>
    </thead>
  );
...
export function ErpConfirmModal({
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className={cn(
        "relative w-full max-w-sm p-6 rounded-[2rem] border shadow-2xl space-y-5 transition-all duration-300",
        darkMode ? "bg-slate-900 border-slate-800 text-white" : "bg-white border-slate-100 text-slate-850"
      )}>
