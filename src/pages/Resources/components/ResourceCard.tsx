    ...
    >
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className={cn("text-[10px] font-black uppercase tracking-widest", darkMode ? "text-slate-500" : "text-slate-400")}>
            {resource.identifier}
          </span>
          <span className={cn(
        ...
        <h4 className={cn("text-sm font-black line-clamp-1 text-left", darkMode ? "text-slate-100" : "text-slate-800")}>
          {resource.name}
        </h4>
        <p className={cn("text-[10px] font-bold text-left", darkMode ? "text-slate-400" : "text-slate-550")}>
        ...
        <div className={cn("pt-3 border-t space-y-2", darkMode ? "border-slate-800/30" : "border-slate-100")}>
          {resource.status === 'MAINTENANCE' ? (
            <div className="flex items-center gap-2 text-rose-500 text-xs font-bold text-left">
              <AlertTriangle className="w-4 h-4 text-rose-500" />
              <span>Đang bảo trì sửa chữa</span>
            </div>
          ) : isOccupied && currentBooking ? (
            <div className={cn("p-3 border rounded-xl space-y-1 text-left", darkMode ? "bg-brand-primary/10 border-brand-primary/20" : "bg-cyan-50/50 border-cyan-100/60")}>
              ...
              <p className={cn("text-[10px] font-black truncate", darkMode ? "text-slate-200" : "text-slate-800")}>
                {currentBooking.purpose}
              </p>
              <p className="text-[9px] font-bold text-slate-500">Đăng ký bởi: {currentBooking.by}</p>
            </div>
          ) : (
            <div className="flex items-center gap-2 text-emerald-500 text-xs font-bold text-left">
