...
export function ImportPartnerModal({ isOpen, onClose, onSuccess }: ImportPartnerModalProps) {
  ...
  const mapHeaders = (row: unknown[]): Record<string, number> => {
    ...
    row.forEach((cell, idx) => {
      ...
      else if (val.includes('owner id') || val.includes('center id') || val.includes('mã trung tâm')) map.centerId = idx;
    });
    return map;
  };
      ...
      try {
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (user?.role === 'superadmin' && !partnerOwnerIdRequired(partnerData)) {
            errors.push('Mã trung tâm (Owner ID) là bắt buộc cho tài khoản superadmin');
          }
                    ...
                    <table className="w-full text-left text-xs min-w-[900px]">
                      ...
                      <tbody className="divide-y divide-slate-100 font-medium">
                        {validationRows.map((row) => (
                          <tr key={row.rowNum} className={`hover:bg-slate-55/30 ${!row.isValid ? 'bg-rose-50/10' : ''}`}>
